...
| MOIS standard Organization codes | `packages/connectors/mois_organization_codes.py` | Contract-only current-institution API parser keyed by seven-digit `org_cd`; preserves hierarchy/lifecycle fields and credentials never persist; no worker, canonical Organization materialization or Gukgam binding is authorized at L1 |
| org.go top-level government Organizations | `packages/connectors/orggo_top_level_organizations.py` | Credential-free official HTML list parser keyed by seven-digit `orgCode` plus `chartId` and provider category; current reviewed live proof is 63 rows across 7 pages; metadata-only, no canonical Organization materialization or Gukgam binding at L2 |

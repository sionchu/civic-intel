...
@dataclass(frozen=True)
class NecWinnerEnumerationResult:
    run: SourceRun
    pages_committed: int
    unique_records: int
...
def normalized_nec_winner(record: NecWinnerRecord) -> dict[str, object]:
    aliases = [
        value
        for value in (record.name_hanja,)
...
class LocalElectionWinnerEnumerator:
    ...
    def enumerate(self, *, resume: bool = False) -> NecWinnerEnumerationResult:
        if any(
            (
        ...
        try:
            ...
            while True:
                ...
                if expected_total is None:
                    ...
                    raise NecWinnerCoverageError("NEC winner total count changed during enumeration")
                ...
                for winner in winners:
                    ...
                    aliases = [
                        value
                        for value in (winner.name_hanja,)
                        if value and value != winner.name_ko
                    ]
                    observations.append(
                        FeederObservation(
                            feeder=self.FEEDER,
            ...
            return NecWinnerEnumerationResult(completed, pages_committed, len(seen_hashes))
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--resume",
        action="store_true",
        help="Resume winner enumeration from the last committed page.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if (args.enumerate_winners or args.resume) and any(
        (args.province, args.district, args.party)
    ):
        parser.error("winner L3 enumeration does not accept province, district, or party filters")
    winner_connector = NecWinnerConnector(
    ...
    try:
        if args.enumerate_winners or args.resume:
